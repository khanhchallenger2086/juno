			...
			<div class="container">
				<div class="heading-pageNew">
					<div class="header-pageCart">
						...
						<span class="count-cart">(Có <b>{{ session()->has('total_buy') ? session()->get('total_buy') : ""  }}</b> sản phẩm trong giỏ hàng)</span>
					</div>
				</div>
				<div class="row">
					<form action="/cart" method="post" id="cartformpage">
						<div class="col-xs-12 col-sm-8">
                            @if ( session()->has('total_buy') &&  session()->get('total_buy') > 0 )
										...
										<table class="table-cart">
											<tbody class="updateItem">
                                                ...
                                                $tongtien += $item_cart->variant['price'] * $item_cart->buy;
                                                 ?>
                                                    <tr class="clearfix line-item-container" data-variant-id="1061095933">
                                                        <td class="image imageVariantCart SD07051">
                                                            <div class="product_image">
                                                                <a href="/products/giay-sandal-phoi-day-vai-soc">
                                                                <img src="/backend/images/juno/{{ explode(";",$item_cart->image_main)[0]  }}" alt="{{$item_cart->name }}">
                                                                </a>
                                                            </div>
                                                        </td>
                                                        <td class="item itemVariantCart">
                                                            ...
                                                            <p class="variant">
                                                                <span class="variant_title">{{$item_cart->variant['size']}} / {{$item_cart->variant['color']}} </span>
                                                            </p>

                                                            {{-- <a href="javascript:void(0)" data-handle="/products/giay-sandal-phoi-day-vai-soc" class="editItem" data-id="1061095933" data-quantity="13" data-line="1">
                                                                Sửa
                                                            </a> --}}
                                                            <span class="delete_cart" data-id="{{$item_cart->variant['id']}}">
                                                                Xóa
                                                            </span>
                                                        </td>
                                                        ...
                                                        <td class="removeInMobile hidden-xs quantity quantityVariantCart partentBtnQuantity">
                                                            <div class="qty quantity-partent qty-click clearfix">
                                                                <button type="button" data-id="{{$key}}" class="qtyminus qty-btn">-</button>
                                                            <input  type="text"   value="{{ $item_cart->buy }}" class="buy tc line-item-qty item-quantity">
                            ...
                            @else
                            <div class="d-none empty-cart span12 expanded-message text-center ">
								Giỏ hàng của bạn đang trống
                            </div>
                            @endif
						</div>
						<div class="col-xs-12 col-sm-4">
							<div class="boxRightCart clearfix">
								...
								<div class="summary-subtotal clearfix">
									<p class="order-infor">
										<span class="titleOrder">Tạm tính:</span>
                                    <span class="valOrder total_price noBold"><b>{{ isset($tongtien) ? number_format($tongtien) . "₫" : "0đ"}}</b></span>
									</p>
									<p class="order-infor">
										<span class="titleOrder">Phí vận chuyển:</span>
										<span class="valOrder">0đ</span>
									</p>
									<p class="order-infor boldPrice">
										<span class="titleOrder"><b>Tổng tiền:</b></span>
										<span class="valOrder total_price"><b>{{ isset($tongtien) ? number_format($tongtien) . "₫" : "0đ"}}</b></span>
									</p>
								</div>
